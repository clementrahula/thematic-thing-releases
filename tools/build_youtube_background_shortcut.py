...
JS = r'''(() => {
  ...
  const visibilityDescriptor = Object.getOwnPropertyDescriptor(DP, 'visibilityState');
  const webkitHiddenDescriptor = Object.getOwnPropertyDescriptor(DP, 'webkitHidden');
  const realHidden = () => {
    try {
  ...
  // Make YouTube believe the page stays visible. This stops its own
  // visibility-based background pause logic without creating a second player.
  ...
  // Ignore page-script pause() calls once WebKit says the page is actually
  // hidden. Native iOS may still issue its own pause; the pause listener above
  // immediately resumes the same media element in that case.
  try {
  ...
  // YouTube is a single-page app and can replace its <video> element.
